...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.layer0 = nn.Sequential(conv1, bn1, relu)
...
def ResNet50(num_classes: int = 10, **kwargs):
    return _resnet(Bottleneck, [3, 4, 6, 3], num_classes, **kwargs)


def ResNet101(num_classes: int = 10, **kwargs):
    return _resnet(Bottleneck, [3, 4, 23, 3], num_classes, **kwargs)


def ResNet152(num_classes: int = 10, **kwargs):
...
if __name__ == '__main__':
    ...
    layers = dict(x.named_modules()).keys()
    extractor = FeatureExtractor(x, layers)
    output = extractor.forward(y)
    for name, mid in output.items():
        print(mid.requires_grad)
